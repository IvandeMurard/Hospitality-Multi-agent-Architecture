...
What compounds is the **Decision Ledger** — the record of decisions and their outcomes, including which manager overrode which recommendation, and who turned out to be right. **Built**: it links each recommendation to the response it got and the outcome that followed, in one auditable object rather than across several tables. Capture of the response was rebuilt in September 2026 and goes live with the next deployment: keywords in French and English, free-text replies read by the model in a validated format, and a confirmation step before anything uncertain is recorded; the decision records its source and who took it. Until that deployment, only an exact accept or reject, or a corrected figure, is recorded. The project says *outcome-tracked operational history*, never *self-improving memory* — the first is checkable today, the second would have to be demonstrated. See [COGNITION](COGNITION.md).
...
2. **Glue, not replacement.** The Mesh uses a PMS‑agnostic canonical schema behind adapters. Intelligence is delivered inside existing tools (e.g., WhatsApp receipts answered with a one-word reply; tap-to-answer buttons are in progress), with zero new dashboards.
