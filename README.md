...
1. **Execution nodes never orchestrate, and never call each other.** Perception nodes (Anima, Aetherix, Tacet) interpret signals; the bespoke Orchestrator holds all decision logic and mediates every exchange. Exceptions are documented and limited to non‑personal, read‑only flows logged the same way — never for Anima. **Today:** with the Orchestrator still Design, wherever Tacet's output reaches Aetherix it does so directly; that is the one standing exception.
